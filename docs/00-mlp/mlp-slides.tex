...
\textbf{1D case:}
$$-\frac{d^2u}{dx^2} = 2, \quad u(0) = u(1) = 0$$
...
\caption{Learning from sparse data: 5 noisy measurements to reconstruct continuous solution}
\end{figure}
...
\textbf{Given:}
\begin{itemize}
    \item Sparse measurements (e.g., 5 points)
...
\begin{tikzpicture}[scale=0.8]
    \node[draw, rectangle] (w) at (-2,0) {Weierstrass (1885)};
    \node[draw, rectangle] (s) at (4,0) {Stone-Weierstrass (1937)};
    \node[draw, rectangle] (k) at (10,0) {Kolmogorov (1957)};
    \node[draw, rectangle] (c) at (2,-2) {Cybenko (1989)};
    \node[draw, rectangle] (h) at (6,-2) {Hornik (1991)};
    
    \draw[->] (w) -- (s) node[midway,above] {generalize};
    \draw[->] (s) -- (k) node[midway,above] {2-layer};
    \draw[->] (s) -- (c) node[midway,right] {neural};
    \draw[->] (c) -- (h) node[midway,below] {any activation};
\end{tikzpicture}
\end{center}
...
\begin{figure}
\centering
% \includegraphics[width=\textwidth]{figs/discontinuous-challenge.png}
...
\begin{itemize}
    \item AND gate
    \item OR gate
    \item Simple classification problems
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{figs/linear-network-failure.png}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{figs/why-activation.png}
...
\begin{frame}{Backpropagation: Mathematical Details}
\textbf{Forward Pass:}
\begin{align}
z^{(1)} &= W^{(1)} x + b^{(1)} \\
a^{(1)} &= \sigma(z^{(1)}) \\
...
\begin{itemize}
    \item $L^2$ regularization (weight decay): \\
    $\mathcal{L}_{total} = \mathcal{L} + \lambda \|\theta\|^2$
    \item $L^1$ regularization (sparsity)
    \item Early stopping
...
\begin{itemize}
    \item Dropout: randomly zero neurons
    \item Batch normalization
    \item Layer normalization
    \item Mixup, cutout, etc.
\end{itemize}
...
\caption{Effect of regularization on decision boundaries}
\end{figure}
\end{columns}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{figs/contradiction-visualization.png}
...
\textbf{Requirements for universal approximation:}
\begin{itemize}
    \item Non-polynomial
    \item Non-constant
...
\normalsize{
krishnak@utexas.edu \\
\url{https://github.com/krishnakumar}
}
\end{center}
\end{frame}
