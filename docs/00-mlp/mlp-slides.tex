...
\textbf{Target:} $\sin(100x)$

\begin{columns}
\column{0.35\textwidth}
\textbf{Shallow (1 layer):}
\begin{itemize}
    \item 100 neurons
    \item Error: 0.15
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figs/shallow-vs-deep-sin100x.png}
\end{figure}
\end{columns}
...
%----------------------------------------------------------------------------------------
\section{Practical Implications for SciML}

\begin{frame}{Physics-Informed Neural Networks}
\textbf{Key idea:} Embed physics in loss function

$$L = L_{data} + \lambda_{PDE} L_{PDE} + \lambda_{BC} L_{BC}$$

\textbf{Components:}
\begin{itemize}
    \item $L_{data}$: Fit observations
    \item $L_{PDE}$: Satisfy differential equation
    \item $L_{BC}$: Enforce boundaries
\end{itemize}

\textbf{Example - Poisson equation:}
$$L_{PDE} = \int_\Omega \left|\frac{\partial^2 u_{NN}}{\partial x^2} + f\right|^2 dx$$

\textbf{Advantages:}
\begin{itemize}
    \item Mesh-free
    \item Handles sparse data
...
\begin{itemize}
    ...
    \item Turbulence closure
\end{itemize}

\textbf{Advantages:}
\begin{itemize}
    \item Continuous solutions
    \item Handle high dimensions
    \item Incorporate data naturally
