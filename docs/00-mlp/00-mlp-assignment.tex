...
\section{Computational Experiments: Function Approximation [3 pts]}
...
\section{Compositional Learning [3 pts]}
...
\section{Regularization in High-Dimensional Spaces [2 pts]}
\subsection{L1 vs. L2 Regularization for Feature Selection}
This problem explores how regularization can prevent overfitting and perform feature selection in a high-dimensional setting. We will consider approximating a simple signal, $y = \sin(x)$, from noisy data points. To make the problem challenging, we will transform the input $x$ into a high-dimensional feature space using 100 Gaussian Radial Basis Functions (RBFs), creating an over-parameterized system. The model is a linear combination of these basis functions, and regularization is added to the standard Mean Squared Error loss:
\[ L_{\text{total}} = \text{MSE} + \lambda \cdot R(W) \]
where $R(W)$ is the regularization penalty on the weights $W$. We will compare:
\begin{itemize}
	\item \textbf{L2 Regularization (Ridge):} $R(W) = \|W\|_2^2 = \sum_i w_i^2$, which penalizes the squared magnitude of the weights.
	\item \textbf{L1 Regularization (Lasso):} $R(W) = \|W\|_1 = \sum_i |w_i|$, which penalizes the absolute magnitude of the weights.
\end{itemize}

\begin{enumerate}
	\item[\textbf{a)}] \textbf{Function Approximation:} 
	\begin{itemize}
		\item Describe the behavior of the model with no regularization. 
		\item How do the L1 and L2 regularized models approximate the underlying true signal?
	\end{itemize}
	\item[\textbf{b)}] \textbf{Learned Weights \& Sparsity:} Examine the plot showing the learned weights (coefficients) for the basis functions.
	\begin{itemize}
		\item Compare the distribution of weight values for the L2 (Ridge) and L1 (Lasso) models. What is the most significant visual difference between the two?
		\item The L1 penalty is known to produce \textbf{sparse} solutions. Explain what sparsity means in this context and how the plot for the L1 model demonstrates it.
	\end{itemize}
\end{enumerate}
