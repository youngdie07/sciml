...
\title{Neural Networks and Function Approximation}
...
%----------------------------------------------------------------------------------------
\section{From Polynomials to Neural Networks: The Weierstrass Approximation Theorem}
...
Every continuous function on a closed interval $[a, b]$ can be uniformly approximated as closely as desired by a polynomial function.
...
This theorem tells us that the set of polynomials is \textbf{dense} in the space of continuous functions $C([a,b])$ under the uniform norm.

\subsection{Why This Matters for Neural Networks}

The Weierstrass theorem establishes a crucial principle: \textbf{simple building blocks can approximate arbitrarily complex continuous functions}. Both polynomials and neural networks follow this principle:

\begin{itemize}
\item \textbf{Polynomials}: Build from monomials $(1, x, x^2, x^3, ...)$
\item \textbf{Neural Networks}: Build from activation functions (ReLU, sigmoid, tanh, ...)
\end{itemize}
...
\subsection{Polynomial Approximation of $\sin(\pi x)$}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{figs/polynomial-approximation-sinpi.png}
\caption{Polynomial approximation of $\sin(\pi x)$ with increasing degrees. The approximation error decreases exponentially as the polynomial degree increases, demonstrating the Weierstrass theorem.}
\label{fig:polynomial-approx}
\end{figure}

\begin{itemize}
\item Degree 3: Max error $\approx 0.05$
\item Degree 5: Max error $\approx 0.008$
\item Degree 7: Max error $\approx 0.001$
\item Degree 9: Max error $\approx 0.0001$
\end{itemize}

The error decreases exponentially with polynomial degree, demonstrating the theorem in action.

\subsection{Bernstein Polynomials: A Constructive Proof}
...
These polynomials:
\begin{itemize}
\item Converge uniformly to $f$ as $n \to \infty$
\item Always stay within the range of the function
\item Form the basis for Bézier curves in computer graphics
\end{itemize}

\begin{figure}[h!]
\centering
\includegraphics[width=0.65\textwidth]{figs/bernstein-approximation.png}
\caption{Bernstein polynomial approximation of $\sin(\pi x)$ for different values of $n$. As $n$ increases, the approximation converges uniformly to the target function.}
\label{fig:bernstein-approx}
\end{figure}

\subsection{Limitations of Polynomial Approximation}
...
\item \textbf{Global Support}: Changing one coefficient affects the entire function
\item \textbf{Computational Instability}: High-degree polynomials suffer from numerical issues
\item \textbf{Poor Extrapolation}: Polynomials diverge rapidly outside the training interval
\end{enumerate}

Neural networks address these limitations:
\begin{itemize}
\item \textbf{Local Support}: ReLU networks create piecewise linear approximations
\item \textbf{Stability}: Bounded activations (sigmoid, tanh) prevent divergence
\item \textbf{Compositionality}: Deep networks build complex functions from simple pieces
\item \textbf{Adaptivity}: Networks learn where to place their basis functions
\end{itemize}

\subsection{Comparison: Polynomials vs Neural Networks}

\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth]{figs/polynomial-vs-nn-comparison.png}
\caption{Direct comparison between polynomial (degree 9) and neural network (10 hidden units with sigmoid activation) approximation of $\sin(\pi x)$. Both achieve good approximation but with different characteristics.}
\label{fig:poly-vs-nn}
\end{figure}

For approximating $\sin(\pi x)$:
\begin{itemize}
\item \textbf{Polynomial (degree 9)}: 10 parameters, max error $\approx 0.0001$
\item \textbf{Neural Network (10 hidden units)}: 21 parameters, max error $\approx 0.001$
\end{itemize}

While both achieve similar accuracy, neural networks:
\begin{itemize}
\item Are more stable numerically
\item Generalize better to new data
\item Scale better to high dimensions
\item Can be composed into deep architectures
\end{itemize}
...
\begin{enumerate}
\item \textbf{1885 - Weierstrass}: Polynomials are universal approximators
\item \textbf{1900s - Stone-Weierstrass}: Generalization to other function algebras
\item \textbf{1957 - Kolmogorov}: Functions of multiple variables
\item \textbf{1989 - Cybenko}: Single-layer neural networks are universal approximators
\item \textbf{1991 - Hornik}: Extension to general activation functions
\item \textbf{Modern Deep Learning}: Deep networks are exponentially more efficient
\end{enumerate}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth]{figs/relu-decomposition-sinpi.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.45\textwidth]{figs/contradiction-visualization.png}
\caption{Proof by contradiction visualization. If neural networks cannot approximate a continuous function, a detector measure $\mu$ must exist. But half-spaces can isolate any point, forcing $\mu = 0$ everywhere—a contradiction since $\mu$ must detect the target function.}
...
\item Deep (20 neurons $\times$ 4 layers): error $\approx 0.03$
\end{itemize}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{figs/derivative-approximation.png}
...
%----------------------------------------------------------------------------------------
\section{Backpropagation: Computing Gradients Efficiently}

\subsection{The Chain Rule Foundation}

Backpropagation is the algorithmic implementation of the chain rule for computing gradients in computational graphs. For a neural network with loss $L$ and parameters $\theta$, we need $\nabla_\theta L$.
...
\begin{algorithmic}
\STATE \textbf{Input:} $x$, parameters $\{W^{(l)}, b^{(l)}\}_{l=1}^L$
\STATE $a^{(0)} \gets x$
\FOR{$l = 1$ to $L$}
    \STATE $z^{(l)} \gets W^{(l)}a^{(l-1)} + b^{(l)}$
    \STATE $a^{(l)} \gets \sigma(z^{(l)})$
\ENDFOR
\STATE \textbf{Output:} $\hat{y} = a^{(L)}$
\end{algorithmic}
...
\subsection{Beyond Symbolic and Numerical Differentiation}

Three approaches to computing derivatives:

\begin{enumerate}
\item \textbf{Symbolic:} Manipulate expressions algebraically
   \begin{itemize}
   \item Exact derivatives
   \item Expression swell problem
   \item Example: $\frac{d}{dx}[\sin(x)] = \cos(x)$
   \end{itemize}

\item \textbf{Numerical:} Finite differences
   \begin{itemize}
   \item $f'(x) \approx \frac{f(x+h) - f(x)}{h}$
   \item Truncation and roundoff errors
   \item $O(n)$ function evaluations for gradient
   \end{itemize}

\item \textbf{Automatic:} Decompose into elementary operations
   \begin{itemize}
   \item Machine precision accuracy
   \item Same cost as function evaluation
   \item Foundation of modern deep learning
   \end{itemize}
\end{enumerate}

\subsection{Forward Mode AD}

Forward mode computes directional derivatives by propagating tangents alongside function values:

\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth]{figs/ad3.png}
\caption{Forward mode automatic differentiation. Each operation computes both the value and its derivative (tangent) simultaneously. The derivative flows forward through the computational graph.}
\end{figure}

For each elementary operation, we compute:
\begin{align}
v_0 &= x, \quad \dot{v}_0 = 1 \\
v_1 &= \sin(v_0), \quad \dot{v}_1 = \cos(v_0) \cdot \dot{v}_0 \\
v_2 &= v_0 \cdot v_1, \quad \dot{v}_2 = \dot{v}_0 \cdot v_1 + v_0 \cdot \dot{v}_1
\end{align}
...
Reverse mode computes gradients by propagating adjoints backward through the computational graph:

\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth]{figs/ad7.png}
\caption{Reverse mode automatic differentiation (backpropagation). After a forward pass computes all intermediate values, gradients are propagated backward from output to inputs using the chain rule.}
\end{figure}

The adjoint computation follows:
\begin{align}
\bar{v}_n &= 1 \quad \text{(seed)} \\
\bar{v}_i &= \sum_{j: i \in \text{parents}(j)} \bar{v}_j \frac{\partial v_j}{\partial v_i}
\end{align}

Cost: One forward pass + one backward pass for full gradient. This is why backpropagation is so efficient for neural networks—we get all parameter gradients in just two passes!
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{figs/ad1.png}
\caption{Computational graph for $f(x_1, x_2) = x_1 x_2 + \sin(x_1)$. Forward mode propagates derivatives from inputs to outputs, while reverse mode propagates gradients from outputs to inputs.}
\end{figure}

The gradients computed via reverse mode:
\begin{align}
...
\subsection{Adaptive Learning Rates (Adam)}
...
%----------------------------------------------------------------------------------------
\section{Experimental Results: Depth vs Width}
...
\begin{itemize}
\item \textbf{Shallow Network:} 100 neurons, 1 hidden layer
   \begin{itemize}
   \item Parameters: 200 (100 input-to-hidden + 100 hidden-to-output)
   \item RMSE: 0.718
   \item Struggles with rapid oscillations
   \end{itemize}

\item \textbf{Deep Network:} 20 neurons per layer, 4 layers
   \begin{itemize}
   \item Parameters: 1240 ($20 + 3 \times 400 + 20$)
   \item RMSE: 0.478 (50\% improvement)
   \item Better captures high-frequency patterns
   \end{itemize}
\end{itemize}
...
\begin{enumerate}
\item \textbf{Hierarchical composition:} Build complex functions from simple pieces
\item \textbf{Exponential expressivity:} Each layer can double the number of linear regions
\item \textbf{Feature reuse:} Intermediate layers learn reusable representations
\end{enumerate}
...
\begin{equation}
L_{\text{physics}} = \frac{1}{N}\sum_{i=1}^N |\mathcal{F}[u_\theta](x_i)|^2
\end{equation}

\subsection{Example: Poisson Equation}

For $-\nabla^2 u = f$ with $u|_{\partial\Omega} = g$:
\begin{align}
L = &\frac{1}{N_{\text{int}}}\sum_{i=1}^{N_{\text{int}}} \left|\frac{\partial^2 u_\theta}{\partial x^2}(x_i) + \frac{\partial^2 u_\theta}{\partial y^2}(x_i) + f(x_i)\right|^2 \\
    &+ \frac{1}{N_{\text{bc}}}\sum_{j=1}^{N_{\text{bc}}} |u_\theta(x_j) - g(x_j)|^2
\end{align}

\subsection{Advantages of PINNs}

\begin{enumerate}
\item \textbf{Mesh-free:} No need for grid generation
\item \textbf{High dimensions:} Avoid curse of dimensionality
\item \textbf{Inverse problems:} Can infer parameters from data
\item \textbf{Irregular domains:} Handle complex geometries naturally
\end{enumerate}
